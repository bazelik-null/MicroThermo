...
void thermostat_setup(thermostat_instance_t* sensor, thermostat_config_t* config)
{
	...
	{
		thermostat_raise_error(sensor, "[ERROR]: Temperature sensor configuration failed.");
	}
...
{
	// Activate conditions
	if (sensor->temperature_inside < sensor->config->MIN_TEMP_INSIDE || sensor->temperature_outside < sensor->config->MIN_TEMP_OUTSIDE) {
		// Activate relay
		gpio_set_level(sensor->config->RELAY_PIN, GPIO_INTR_HIGH_LEVEL);
		gpio_set_level(sensor->config->RELAY_LED_PIN, GPIO_INTR_HIGH_LEVEL);
	}

	// Deactivate conditions
	else if (sensor->temperature_inside > sensor->config->MAX_TEMP_INSIDE && sensor->temperature_outside > sensor->config->MAX_TEMP_OUTSIDE) {
		// Deactivate relay
		gpio_set_level(sensor->config->RELAY_PIN, GPIO_INTR_LOW_LEVEL);
		gpio_set_level(sensor->config->RELAY_LED_PIN, GPIO_INTR_LOW_LEVEL);
	}
}
...
void thermostat_raise_error(thermostat_instance_t* sensor, const char* error)
{
	...
	// Activate error LED
	gpio_set_level(sensor->config->ERROR_LED_PIN, GPIO_INTR_HIGH_LEVEL);

	if (sensor->config->IS_DEBUG)
	{
		uart_write(error);
	}
...
void thermostat_update(thermostat_instance_t* sensor)
{
	...
	{
		thermostat_raise_error(sensor, "[ERROR]: Temperature cannot be read.");
	}
	...
	// If temperature is under or above 100C something is certainly wrong
	if (sensor->temperature_inside < -100 || sensor->temperature_outside < -100 || sensor->temperature_inside > 100 || sensor->temperature_outside > 100)
	{
		thermostat_raise_error(sensor, "[ERROR]: Temperature sensor malfunction detected.");
	}
}